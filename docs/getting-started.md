...
If you ran `knarr init` or `knarr use`, this can happen automatically via the `postinstall` hook when `knarr` is available on the install script `PATH` (for example, installed globally or as a devDependency).

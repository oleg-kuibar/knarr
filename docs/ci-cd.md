...
- The `postinstall` hook (`knarr restore --silent || true`) is safe in CI -- if Knarr is not available on the install script `PATH`, `|| true` prevents the hook from failing. Install Knarr as a devDependency or run `npx knarr restore` explicitly when CI needs automatic restore.

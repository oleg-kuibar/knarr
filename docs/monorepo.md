...
Running `pnpm install` in a workspace can wipe `node_modules/` overrides. If `knarr init` was run in each consumer, the `postinstall` hook (`knarr restore --silent || true`) will re-inject automatically.
